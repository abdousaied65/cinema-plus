    ...
    <div class="row mailbox">
        <div class="col-lg-3 col-md-4">
            ...
            <ul class="list-group list-group-divider inbox-list">
                ...
                <li class="list-group-item">
                    <a href="{{route('admin.contacts.sent')}}"><i class="fa fa-envelope-o"></i> {{trans('msgs.Sent')}}
                        <i
                ...
                <div class="mailbox-header">
                    <div class="d-flex justify-content-between">
                        <h5 class="d-none d-lg-block inbox-title ml-3"><i
                                class="fa fa-envelope-o m-r-5"></i> {{trans('msgs.Sent')}} ({{$data->total()}})
                        </h5>
                            ...
                            <div class="d-flex" style="padding: 10px;">
                                ...
                                <div id="inbox-actions" class="m-l-20 m-r-20">
                                    <form class="d-inline" method="post">
                                        @csrf
                                        @method('PATCH')
                                        <button formaction="{{route('admin.contacts.make.sent.as.destroy')}}" type="submit"
                    ...
                    <table class="table table-hover table-inbox" id="table-inbox">
                        ...
                        @foreach($data as $msg)
                            <tr>
                                ...
                                <td class="@if($msg->status == 1) text-muted @elseif($msg->status == 2) text-success  @endif">
                                    @if(filter_var($msg->email, FILTER_VALIDATE_EMAIL))
                                        {{$msg->email}}
                                    @else
                                        {{trans('msgs.'.$msg->email.'')}}
                                    @endif
                                </td>
