...
@section('content')
    ...
    <div class="row row-sm">
                    ...
                    <div class="d-flex justify-content-between">
                        <div class="col-lg-12 margin-tb">
                            ...
                            <a class="btn pull-right btn-danger btn-md" style="margin-right: 20px"
                               href="{{ route('admin.stars.trashed') }}">
                                <i class="fa fa-eye"></i> {{trans('msgs.Show Trashed Star')}} </a>
                            <h5 style="min-width: 300px;"
                                class="pull-left alert alert-sm alert-success">{{trans('msgs.Display All Stars')}}</h5>
                        </div>
                        <br>
                    </div>
                </div>
                                ...
                                <tr class="text-center">
                                    ...
                                    <td>
                                        <a class="btn btn-primary btn-md"
                                           href="{{ route('admin.stars.edit', $star->id) }}"><i
                                                class="fa fa-pencil"></i> {{trans('msgs.Edit')}} </a>

                                        <a class="modal-effect btn btn-md btn-danger delete_star"
                                           star_id="{{ $star->id }}"
                                           star_name="{{ $star->name }}" data-toggle="modal" href="#modaldemo9"
                                           title="Delete"><i
        ...
        <div class="modal" id="modaldemo9">
            <div class="modal-dialog modal-dialog-centered" star="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header text-center">
                        <h6 class="modal-title w-100"
                            style="font-family: 'Cairo'; ">{{trans('msgs.Delete Star')}}</h6>
                        <button aria-label="Close" class="close"
                                data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                    </div>
                    <form action="{{ route('admin.stars.destroy', 'test') }}" method="post">
                        {{ method_field('delete') }}
                        {{ csrf_field() }}
                        <div class="modal-body">
                            <p>{{trans('msgs.Are You Sure You Want To Delete')}} ?</p><br>
                            <input type="hidden" name="star_id" id="star_id" value="">
                            <input class="form-control" name="starname" id="starname" type="text" readonly>
                        </div>
...
<script>
    $(document).ready(function () {
        $('.delete_star').on('click', function () {
            var star_id = $(this).attr('star_id');
            var star_name = $(this).attr('star_name');
            $('.modal-body #star_id').val(star_id);
            $('.modal-body #starname').val(star_name);
        });
    });
</script>
